 ...
 * Description:拖拉拽组件
 * @author:danzizhong
 * GitHub：https://github.com/zsz-danzi/panel-drag
 * Time：2021.03.10
=================================*/
...
function PlaneDrag(opt) {
  ...
  this.head = this.dom.querySelector(opt.head);
  this.index = opt.index || 1;
  this.life = true;
  this.isfullWindow = opt.isfullWindow ?? true;
  this.opt = opt;
  ...
  self.head.addEventListener(
    ...
    (ev) => {
      ...
      if (this.nowTime - this.prevTime < 300) {
        this.isfullWindow && this.fullWindow();
        return;
      }
...
PlaneDrag.prototype.fullWindow = function (ev) {
  if (!this.fullFlag) {
    ...
    this.left = 0;
    this.dom.style.cssText =
      "-webkit-transition:all ease .3s 0s; width: " +
      this.width +
      "px; height: " +
      this.height +
      "px; transform: translate(" +
...
PlaneDrag.prototype.winMove = function (ev) {
  ...
  if (
    ev.pageX <= r &&
    ev.pageX >= r - this.edge - 7 &&
    ev.pageY >= t &&
    ev.pageY <= t + this.edge + 7
  ) {
  ...
  if (
    ev.pageX >= l &&
    ev.pageX <= l + this.edge + 7 &&
    ev.pageY <= b &&
    ev.pageY >= b - this.edge - 7
  ) {
